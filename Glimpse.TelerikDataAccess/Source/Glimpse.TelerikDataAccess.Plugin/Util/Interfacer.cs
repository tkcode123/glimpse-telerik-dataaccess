...
namespace Glimpse.TelerikDataAccess.Plugin.Util
{
    public static class Interfacer
    {
            ...
            foreach (var z in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                if (t != null)
                {
                    ...
                    if (m != null && m.Length > 0)
                    {
                        ...
                        if (pi != null)
                        {
                            pi.SetValue(null, Util.Interfacer.Create(pi.PropertyType, instance, OnError), null);
                            return true;
                        }
                        var fi = m[0] as FieldInfo;
                        if (fi != null)
                        {
                            fi.SetValue(null, Util.Interfacer.Create(fi.FieldType, instance));
                            return true;
                        }
        ...
        public class InterfaceWrapperBase : IInterfaceWrapper
        {
            ...
            object IInterfaceWrapper.SetWrapped(object o)
            {
                wrapped = o;
                return this;
            }
